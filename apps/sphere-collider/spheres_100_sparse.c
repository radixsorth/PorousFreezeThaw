...
int save_snapshot(int snap, FLOAT *pos)
/* saves the particle positions "pos" to snapshot with number "snap" */
{
	FILE * f;
	int i;
	char filename[1024];
	...
	// output header
	fprintf(f,"x,y,z\n");
	// output particle positions stored in 
	for(i=0;i<n;i++)
		fprintf(f,"%f,%f,%f\n", VEC(pos,i)[0], VEC(pos,i)[1], VEC(pos,i)[2]);
	fclose(f);
}
