...
*                                                   *
* (C) 2005-2014 Pavel Strachota                     *
* file: RK_MPI_SAsolver.h                           *
\***************************************************/

#if !defined __RK_MPI_SAsolver
#define __RK_MPI_SAsolver
...
These vectors need not form a continuous block of memory. Instead, they may be split to several
smaller blocks spread over some restricted area of memory. The usual application of this feature is to
create holes in the memory blocks, which do not belong to the solution, but are used by the right hand
side. This style of memory organization simplifies the formulas in the right hand side e.g. when
dealing with boundary conditions of PDEs converted to a system of ODEs by the method of lines.
------------------------------------------------------------------------------------------------------
The following structures define all the necessary data for a separate equation system solution. Thanks
to that, calculations of more than one equation system can be in progress at the same time (of course,
only one of them running at the moment), provided that all the equations fit into a contiguous memory
block of the size passed to RK_MPI_SA_init(). Before RK_MPI_SA_solve() can be called to carry out the
calculation, all of the following must be ready:
1) The MPI run time environment must be initialized by a call to MPI_Init();
2) The sover must be initialized by a call to RK_MPI_SA_init()
3) The system memory organization structure RK_MEM_DIST must be set up, including the referenced arrays,
   for the correct portion of the particular equation system.
4) the RK_MPI_S_SOLUTION structure must be set up for the particular equation system

NOTE:	'h' is the initial time step. If 'h==0', then the time step will be calculated
	automatically from the value of 'final_time-t' (final_time is passed to RK_MPI_SA_solve).
...
IMPORTANT:
In 'h', RK_MPI_SA_solve() always stores the value of the last estimated time step BEFORE the final trimmed
time step that finishes the calculation right at final_time. This allows multiple calls to
RK_MPI_SA_solve() with the time step being maintained from the previous reasonable estimates.
...
'f' has 3 arguments: 'f(t,x,dest)' and must fill the correct portions (generally, the same chunks as
specified to the RK solver) of the array 'dest', that has the same dimension as 'x', with the values
of the right hand side. The internal structure and array handling inside 'f' is left up to user's
responsibility. 'x' is a pointer to const and thus the elements of th array cannot be modified
from inside 'f'. The system memory distribution structure is not passed to 'f', the user should handle
it correctly by his own means. 'f' must not suppose that the parameter 'x' has any relationship with the
location of the solution 'x' in this structure !!!
...
to RK_MPI_SA_init(). Before the calculation begins, the ODE system initial condition must be distributed
among the processes.
...
nodes of the adjacent blocks. Taking advantage of the sparse memory organization, these values may
be stored directly into the array denoted by the formal parameter 'x', provided that enough space
has been reserved by the call to RK_MPI_SA_init and that the chunks are properly organized.
As a result, the right hand side formula may use the memory as if there were no boundaries between blocks.
...
/*
RK_MEM_DIST defines the system memory distribution. The portions of the whole array 'x' actually used
by the RK solver are referred to as 'chunks'. The chunks must follow one after another in the sense of
increasing memory address, must not overlap and must not exceed the total size allocated.
The above requirements may be checked manually using the RK_MPI_SA_check_mem() function. No such thorough
checking is performed before the start of the calculation in order to save time. Only the last chunk is checked
in order to discover most common memory errors.
*/
typedef struct {
	int n_chunks;					/* the number of chunks in the current rank */
	int * chunk_start;				/* pointer to an array of n_chunks values specifying
							   the offsets of where the chunks start (relative to
							   the beginning of the array 'x' (see below) */
	int * chunk_size;				/* pointer to an array of n_chunks values specifying
							   the sizes of the respective chunks */
	FLOAT * chunk_eps_mult;				/* multiplier of the relative error (eps) for the
							   current chunk. By this, different parts of the
							   equation system can be given different error tolerance. */
} RK_MEM_DIST;

typedef struct __struct_RK_MPI_S_SOLUTION {
	RK_MEM_DIST * n;				/* the memory organization of the system chunks to be processed.
							   This replaces the number of equations solved by the current rank,
							   which is present in the RK_MPI_SOLUTION struct. (see RK_MPI_Asolver.h)
							   The solver does not have to know the size of the whole
							   system at all, nor does it anyhow operate with the overall size
							   of all chunks. */
	...
	FLOAT delta;					/* the maximum desired truncation error (only needed by the master rank) */
	enum { DELTA_LOCAL, DELTA_GLOBAL } delta_mode;	/* truncation error treatment mode:
							   - DELTA_LOCAL interprets delta as the upper bound for the local truncation
							     error (per-time-step error).
							   - DELTA_GLOBAL interprets delta as the upper bound for the (sort of)
							     component-wise rate od departure of the numerical solution
							     from the precise solution (theoretical error accumulated per unit time if
							     the epsilon stayed constant).
							   (only needed by the master rank) */

	/* DDLBF callback function (please read their description carefully) */

	RK_MEM_DIST * (* DDLBF_Rearrange)(RK_MEM_DIST *); /* this is called after each calculation of the next time level
							   ...
							   This function is passed the current pointer to the memory distribution
							   structure (RK_MEM_DIST) and it must return a new pointer to RK_MEM_DIST.
							   Directly modifying the contents of the structure is also possible
							   and takes effect immediately. If the block size is not to be changed,
							   DDLBF_Rearrange() simpy returns the same value that has been passed to it,
							   without modifying anything.

							   This function is also responsible for the following:
							   - send the elapsed time to the master process OR (if master)
							     ...
							     NOTE:	The rank of the 'master' process mentioned here that
									...
									(the rank passed to RK_MPI_SA_init()).
	...
	int (* Service_Callback)(FLOAT, const struct __struct_RK_MPI_S_SOLUTION * const);
							/* a service callback that may optionally provide logging capabilities
							   ...
							   computation time as given in the master rank to RK_MPI_SA_solve().
	...
	long steps_total;				/* the number of time steps performed by the solver, including
							   the unsuccessful ones (useful for statistics & debugging) */
} RK_MPI_S_SOLUTION;
...
/*
Checks whether the given system memory distribution is defined correctly.
For the chunk placement rules, see the definition of RK_MEM_DIST.
